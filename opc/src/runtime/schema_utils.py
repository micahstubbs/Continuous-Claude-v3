...
def sanitize_name(name: str) -> str:
    """
    Sanitize name for Python identifier with security validation.

    This function performs security validation to prevent code injection
    attacks via malicious tool/server names (F2 mitigation, CVSS 8.8).
    ...
    Raises:
        SecurityValidationError: If the name contains dangerous characters
    ...
    Examples:
        ...
        'list_'
    """
    from opc.src.security.validators import validate_identifier, SecurityValidationError

    # First, check for dangerous characters that could enable code injection
    # This catches attacks like: x"); os.system("evil")#
    dangerous_chars = {'"', "'", "`", "\n", "\r", ";", "(", ")", "[", "]", "{", "}"}
    found_dangerous = dangerous_chars & set(name)
    if found_dangerous:
        raise SecurityValidationError(
            f"Name contains dangerous characters: {found_dangerous}",
            "tool_name",
            name
        )
    ...
    # Final validation - ensure the result is a safe identifier
