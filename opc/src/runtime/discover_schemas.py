...
async def write_discovered_types(
    ...
    Raises:
        SecurityValidationError: If server_name contains path traversal chars
    """
    # Security: Validate server_name to prevent path traversal (F4 mitigation, CVSS 4.9)
    from opc.src.security.validators import validate_path_component
    validate_path_component(server_name, field_name="server_name", base_dir=output_dir)
